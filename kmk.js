...
async function run() {
  ...
  const LOGIN_SELECTOR = '#new_user > div.actions > div.col-xs-12.col-md-10.col-md-offset-1.padding-bottom-2 > input';
  ...
  */
  ...
  for (j = 0; j < topic_list.length; j++) {
    ...
    console.log("Processing: " + element[0] + ' ' + element[1] + ' ' + element[2]);

    await page.goto(element[2]);
    ...
    ref.on("value", function(cards) {
      ...
      if (old_cards != null) {
        ...
        total_count = total_count + Object.keys(old_cards).length;
    ...
    //browser.close();
  console.log("Exiting topic loop...");
}
